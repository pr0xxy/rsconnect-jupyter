...
from rsconnect import api, bundle
...
def verify_server(data, contents_manager, log):
    ...
    try:
        canonical_address = api.verify_server(server_address, disable_tls_check, cadata)
...
def app_search(data, contents_manager, log):
    ...
    try:
        retval = api.app_search(uri, api_key, title, app_id, disable_tls_check, cadata)
